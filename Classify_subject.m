...
%%
disp(size(bt))
lbls = cellfun(@num2str,arrayfun(@num2cell,1:20),'UniformOutput',0)';
arr = bt;
arr = arr - arr(:,11,:,:);
arr = arr(:,1:10,:,:);
arr = zscore(arr,[],1);
...
clear ds
for r_ind = 1:size(arr,1)
for s_ind = 1:size(arr,3)
for run_ind = 1:size(arr,4)
row = row+1;
ds.samples(row,:) = arr(r_ind,:,s_ind,run_ind);
ds.sa.ROI(row,1) = r_ind;
