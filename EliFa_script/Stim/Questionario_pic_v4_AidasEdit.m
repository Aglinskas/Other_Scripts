...
%% Set up Java Robot for response hack
import java.awt.Robot
import java.awt.event.*
keys = Robot;
keys.setAutoDelay(40)
...
%     image(ima,'alphadata',1-double(all(mask,3)));
    ...
    [nxp, nyp, textbounds] = DrawFormattedText (w, 'MANGIATO= 1; MAI MANGIATO= 0', ...
    ...
    if myTrials(j).eaten == 1
        myTrials(j).known = 1;
            ...
            % Aidas Edit: Java Robot 
            keys.keyPress(KeyEvent.VK_ENTER) %Flushes whatever input was in the command line
            keys.keyRelease(KeyEvent.VK_ENTER)
            keys.keyPress(KeyEvent.VK_QUOTE) % Surrounds cursor with quotes
            keys.keyRelease(KeyEvent.VK_QUOTE)
            keys.keyPress(KeyEvent.VK_QUOTE)
            keys.keyRelease(KeyEvent.VK_QUOTE)
            keys.keyPress(KeyEvent.VK_LEFT)
            keys.keyRelease(KeyEvent.VK_LEFT)
            Screen('PutImage', w, ima); % keeps image on the screen
            [string,terminatorChar] = GetEchoString(w,'NAME OF FOOD:',textbounds(1),textbounds(2)) % Gets the textbox
            Screen(w,'flip'); % flips both (even though only the image needs fliping, GetEchoString autoplips on execute)
            myTrials(j).response = string; %Keeps Response
            % End of Aidas Code
    end
    ...
    if myTrials(j).eaten == 0
        ...
        end % ends if know, then ask what, loop
    end %ends if eaten loop
