...
% ds = all_scans;
%% Decoding, correlations instead now
MVPA_cormat(which_roi,pairs(paircomp,1),pairs(paircomp,2),subID) = mean(get_triu(corr(all_scans.samples')));%corr(all_scans.samples');
MVPA_cormat(which_roi,pairs(paircomp,2),pairs(paircomp,1),subID) = mean(get_triu(corr(all_scans.samples')));

MVPA_cormat_Raw(which_roi,pairs(paircomp,2),pairs(paircomp,1),subID,:,:) = corr(all_scans.samples');
MVPA_cormat_Raw(which_roi,pairs(paircomp,2),pairs(paircomp,1),subID,:,:) = corr(all_scans.samples');
