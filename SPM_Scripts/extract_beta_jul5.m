clear all
loadMR
bt_fn_temp = '/Users/aidasaglinskas/Google Drive/Data_words/S%d/Analysis/beta_%s.nii';
...
mat = [];
for s_ind = 1:length(w_subvec);
    subID = w_subvec(s_ind);
for m_ind = 1:18
m_fn = fullfile(masks.dir,masks.nii_files{m_ind});
disp(sprintf('subject %d/%d mask %d/18',s_ind,length(subvec),m_ind));
for t_ind = 1:12
