function White_schema(strNames, corrMatrix, fontsize, positive_color_hs, negative_color_hs, theta)
...
%
	...
	if nargin<5
		positive_color_hs = [0.8333 1]%[0.1587 0.8750]; % yellow
		negative_color_hs = [1 1]; % magenta
	end
	...
	for ii=1:M
		rectangle('Curvature',[1 1],'edgeColor',clr(ii,:),...
			'Position',[Px(ii)-markerR Py(ii)-markerR 2*markerR*[0 0]]);
	end
