...
con_clustering{avg_counter} = figure(9+avg_counter);
drawnow
...
unc_clustering{avg_counter} = figure(20+avg_counter);
dendrogram(Z,length(matrix_to_permute),'Colorthresh',2,'Orientation','left')
ground_ord{avg_counter} = str2num(unc_clustering{avg_counter}.CurrentAxes.YTickLabel);
these_lbls = all_labels{find(cellfun(@(x) isequal(length(x),length(avgs_all{avg_counter})),all_labels))};
[t tt] = dendrogram(Z,length(labels_for_the_matrix),'labels',these_lbls,'Colorthresh',.2,'Orientation','left')
...
unc_clustering{avg_counter}.CurrentAxes.YAxis.FontSize = 20
end
drawnow
figure(90)
end
