function boot_func_fixedeffects(matrix,labels,analysis_name)
% boot_func_fixedeffects(matrix,labels,analysis_name)
% Bootstraps matrices, as fixed effects
% Averaged across dim 3 then correlates. 
...
if length(labels_for_the_matrix) == length(corr(avg_matrix_to_permute_features))
avg_matrix_to_permute = corr(avg_matrix_to_permute_features);
elseif length(labels_for_the_matrix) == length(corr(avg_matrix_to_permute_features'))
avg_matrix_to_permute = corr(avg_matrix_to_permute_features')
else error (sprintf('labels|matrix size mismatch: %d labels, for a matrix with size %d/%d',length(labels_for_the_matrix),length(corr(avg_matrix_to_permute_features)),length(corr(avg_matrix_to_permute_features'))))
end
...
if length(corr(squeeze(Bootstrapedkeep(perm,:,:)))) == length(avg_matrix_to_permute)
   tempK =  corr(squeeze(Bootstrapedkeep(perm,:,:)));
elseif length(corr(squeeze(Bootstrapedkeep(perm,:,:))')) == length(avg_matrix_to_permute)
    tempK =  corr(squeeze(Bootstrapedkeep(perm,:,:))');
else error('something dun got fucked, size mismatch at tempK, so sort it out')
end
clear newVec
...
% separate PNGs
arrayfun(@(x) export_fig(['/Users/aidasaglinskas/Desktop/2nd_Fig/Boot_func_output/' 'FixedEffects' analysis_name tm '_' num2str(x.Number) '.png'],x),figs,'UniformOutput',0)
% single PS
