...
this_mat = all_noise{roi_or_task}%all_keeps{roi_or_task};
...
%split_subs(:,1) = split_pool(randi(10,10,1),1); % drop these no use
%length(unique(split_subs))
%split_subs(:,2) = split_pool(randi(10,10,1),2);

split_subs(:,1) = split_pool(:,1);
split_subs(:,2) = split_pool(:,2);
