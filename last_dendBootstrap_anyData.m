...
label(rows2exclude)=[];
labels_for_the_matrix = label% labels for the matrix
%repmat({'No lbl'},1,1138)
part_of_iteration = 1; % is this part of it?
threhold = 25; % only for iterations;
numClust=30; %number of clusters we care about
nperms = 100; %how many permutations?
plot_confmat_and_schemaball = 0 % Might Crash with > 1k^2 sized matrix
%% End of parameters; Code below should run as-is;
% Reset Random numbers
if length(size(matrix_to_permute)) ~= 3
    error('I don''t know how to work with this matrix, I need a 3d correlation matrix: Matrix(SUBJECT,rows,cols)')
elseif length(labels_for_the_matrix) ~= size(matrix_to_permute,2)
    error(sprintf('ERRROR: number of labels doesn''t match the matrix size: %d labels, matrix is %d x %d',length(labels_for_the_matrix),size(matrix_to_permute,2),size(matrix_to_permute,3)))
elseif size(matrix_to_permute,2) ~= size(matrix_to_permute,3)
    error('Not a symetrical matrix') 
end    
if part_of_iteration == 1
    load('/Users/aidasaglinskas/Desktop/dot_mat_files/nperms100.mat');
    load('/Users/aidasaglinskas/Desktop/dot_mat_files/fidelity.mat')
    if length(fidelity.score) ~= length(labels_for_the_matrix)
        error('Fidelity score doesnt match the labels, turn off iterations')
    else %
        orig_size = length(labels_for_the_matrix);
        matrix_to_permute = matrix_to_permute(:,find(fidelity.score > threhold),find(fidelity.score > threhold));
        labels_for_the_matrix = labels_for_the_matrix(find(fidelity.score > threhold));
        warning(sprintf('Matrix has been tresholded at %d percent, resulting matrix size is %d out of the original %d',threhold,length(labels_for_the_matrix),orig_size))
    end
end
rng(randi(100))
rng
...
subject_pool(i,:) = randi([1 size(matrix_to_permute,1)],[1 size(matrix_to_permute,1)]);
end
toc
%%
tic
...
%reducedBetaArray=(subBetaArray(which_rois_to_cor,which_tasks_to_cor,subjects));%subBetaArray(ROI,TASK,SUB)
% Collect bootstrap subjects
Bootstrapedkeep(s,:,:)= squeeze(mean(matrix_to_permute(subjects,:,:),1));
...
avg_matrix_to_permute = squeeze(nanmean(matrix_to_permute,1));
clear newVec
...
dendrogram(Z,length(labels_for_the_matrix),'Colorthresh',2)
ground_ord = str2num(unc_clustering.CurrentAxes.XTickLabel)
dendrogram(Z,length(Z),'labels',labels_for_the_matrix,'Colorthresh',2)
...
tempK = squeeze(Bootstrapedkeep(perm,:,:));
clear newVec
...
for r = 1:size(matrix_to_permute,2)
    for c = 1:size(matrix_to_permute,3)
...
figure(11);schemaball(labels_for_the_matrix,clustering_matrix ./ 100)
end
%%
disp('Sorting Matrix')
...
for i = list'
inds = find(gr == list(i));
temp_mat(i,:) = squeeze(mean(clustering_matrix_sorted(inds,:),1));
end
for i = list'
inds = find(gr == list(i));
clust_clust_mat(:,i) = mean(temp_mat(:,inds),2);
end
...
tm = datestr(datetime);
cellfun(@(x) export_fig(['/Users/aidasaglinskas/Desktop/dot_mat_files/' tm '.pdf'],'-append',x),figs,'UniformOutput',0)
catch
